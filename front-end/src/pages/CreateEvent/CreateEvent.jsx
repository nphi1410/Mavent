...
const CreateEvent = ({ isUpdatePage = false }) => {
  ...
  const validateForm = () => {
    ...
    if (!startDatetime)
      return setError("Thời gian bắt đầu không được để trống.");
    if (!endDatetime)
      return setError("Thời gian kết thúc không được để trống.");
    ...
    if (maxMemberNumber < 0)
      return setError("Số thành viên tối đa không được âm.");
    if (maxParticipantNumber < 0)
      return setError("Số người tham gia tối đa không được âm.");
