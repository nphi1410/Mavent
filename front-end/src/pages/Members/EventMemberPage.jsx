...
const EventMemberPage = () => {
      ...
      try {
        ...
        if (response && response.content) {
          ...
          if (Array.isArray(response.data)) {
            ...
            totalElementsCount =
              response.data.totalElements || memberData.length;
          } else {
        ...
        if (hasFilter) {
          ...
          if (statusFilter) {
            ...
            const activeCount = transformedMembers.filter(
              (m) => m.isActive === wantActive
            ).length;
          ...
          if (shouldFilter) {
            ...
            if (departmentFilter) {
              const isDepartmentId =
                !isNaN(Number(departmentFilter)) &&
                Number(departmentFilter) > 0;

              if (isDepartmentId) {
                const deptId = Number(departmentFilter);
                finalMembers = finalMembers.filter(
                  (m) => m.departmentId && Number(m.departmentId) === deptId
                );
              } else {
                ...
                finalMembers = finalMembers.filter(
                  (m) =>
                    m.department &&
                    m.department.toLowerCase().includes(deptName)
                );
              }
            }
        ...
        setTotalPages(
          hasFilter && shouldFilter
            ? Math.ceil(finalCount / itemsPerPage)
            : totalPagesCount
        );
        setTotalElements(
          hasFilter && shouldFilter ? finalCount : totalElementsCount
        );
  ...
  const handleEditInputChange = (field, value, depts) => {
    ...
    setEditedUser((prev) => ({
      ...
      ...(field === "departmentId" &&
        ...
        Array.isArray(depts) && {
          departmentName:
            depts.find((d) => d.departmentId === value)?.name || "Unknown",
        }),
    }));
  };
  ...
  const handleBulkBanUsers = async () => {
    ...
    try {
      ...
      // Show success toast
      showToast(
        `Đã cấm ${selectedMembers.length} thành viên thành công`,
        "success"
      );
      return true;
    } catch (err) {
  ...
  const handleSelectMember = (memberId, event) => {
    if (event.nativeEvent.shiftKey && selectedMembers.length > 0) {
      ...
      const lastSelectedIndex = memberIds.indexOf(
      ...
      <MemberDetailModal
        ...
        isBanned={
          selectedUser && selectedUser.id
            ? bannedUsers[selectedUser.id] || false
            : false
        }
