...
const formatCurrency = (value) => {
  ...
  const numericValue =
    typeof value === "bigint" ? Number(value) : parseInt(value);
    ...
    <div className="bg-gradient-to-br from-blue-200 to-blue-500 rounded-2xl shadow-lg text-white p-8 lg:p-10">
      <div className="flex items-center justify-between">
        <div>
          ...
          {sourceText && <p className="text-blue-100 text-lg">{sourceText}</p>}
        </div>
        {subtitle && (
...
function VerticalBarChart({ title, data }) {
  ...
  const chartData = {
    labels: data.map((d) =>
      String(d.categoryName || d.departmentName || d.label)
...
function HorizontalBarChart({ title, data }) {
  ...
  const chartData = {
    labels: data.map((d) =>
      String(d.departmentName || d.categoryName || d.label)
    ), // Chuyển đổi thành String
    datasets: [
      {
        label: "Tổng số tiền",
    ...
    datasets: [
      {
        ...
        backgroundColor: data.map(
          (d) => chartJsColors[d.status] || "rgba(107, 114, 128, 0.8)"
        ),
        borderColor: data.map(
          (d) =>
            chartJsColors[d.status].replace("0.8", "1") ||
            "rgba(107, 114, 128, 1)"
        ),
        borderWidth: 1,
      },
            ...
            <div className="flex items-center gap-3">
              <div
                className={`w-3 h-3 rounded-full ${
                  tailwindDotColors[item.status] || "bg-gray-500"
                }`}
              ></div>
...
function AdminViewExpense() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setError(
          "Không thể tải dữ liệu. Vui lòng kiểm tra kết nối mạng hoặc server."
        );
        console.error(err);
      } finally {
        setLoading(false);
  ...
  const currentTotal = totalSpentData
    ? parseInt(totalSpentData.totalAmount)
    : 0;
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 lg:p-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            ...
            <ExpenseExportButton
              eventId={id}
              fileName={`expense_report_event_${id}`}
            />
          </div>
        </div>
