...
export default function ExpenseRequestHistory() {
    ...
    const filterExpenses = () => {
      ...
      if (categoryFilter) {
        filtered = filtered.filter(expense => 
          expense.categoryId === Number(categoryFilter)
        );
      }
      ...
      if (searchTitle) {
        filtered = filtered.filter(expense => 
          expense.note && expense.note.toLowerCase().includes(searchTitle.toLowerCase())
        );
      }
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span
        className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${statusStyles[status] || "bg-gray-500 text-white"}`}
      >
        {status}
      </span>
    );
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          <div className="p-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-6 items-start sm:items-end">
              ...
              {(role.toLowerCase() === "member" || role.toLowerCase() === "department_manager") && (
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {filteredExpenses.map((expense) => (
                        <tr key={expense.expenseId}>
                          ...
                          {(role.toLowerCase() === "admin" || role.toLowerCase() === "department_manager") && (
