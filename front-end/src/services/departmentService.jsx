...
export const getMemberCount = async (eventId, departmentId) => {
  try {
    ...
    const response = await Api.get(
      `/events/${numericEventId}/departments/${numericDepartmentId}/member-count`
    );
    ...
    console.error(
      `Error fetching member count for department ID ${departmentId}:`,
      error
    );
...
export const deleteDepartment = async (eventId, departmentId) => {
  try {
    ...
    console.log(
      `Attempting to delete department: eventId=${numericEventId}, departmentId=${numericDepartmentId}`
    );
    ...
    try {
      ...
      const response = await Api.delete(
        `/events/${numericEventId}/departments/${numericDepartmentId}`
      );
      return response.data;
    } catch (pathError) {
      ...
      if (pathError.response && pathError.response.status === 500) {
        ...
        try {
          const altResponse = await Api.delete(
            `/events/${numericEventId}/departments`,
            {
          ...
          if (altError.response?.data?.message) {
            ...
            errorMessage =
              "Không thể xóa phòng ban. Lỗi xuất hiện do ràng buộc cơ sở dữ liệu.";
          }
    ...
    if (error.response) {
      ...
      if (status === 404) {
        ...
        console.log(
          "Server returned error 500 with details:",
          error.response.data
        );
        throw new Error(errorMessage);
      }
...
export const checkDepartmentHasTasks = async (eventId, departmentId) => {
  try {
    ...
    try {
      ...
      const response = await Api.get(
        `/events/${numericEventId}/departments/${numericDepartmentId}/task-count`
      );
      return {
...
export const forceDeleteDepartment = async (eventId, departmentId) => {
  try {
    ...
    console.log(
      `Warning: Attempting to force delete department: eventId=${numericEventId}, departmentId=${numericDepartmentId}`
    );

    // Try to use a special force delete endpoint if the backend provides one
    try {
      const response = await Api.delete(
        `/events/${numericEventId}/departments/${numericDepartmentId}/force`
      );
      return response.data;
    } catch (pathError) {
      ...
      try {
        ...
        throw new Error(
          "Không thể xóa phòng ban, ngay cả với cờ force. Liên hệ quản trị viên hệ thống."
        );
      }
    }
  } catch (error) {
...
export const getSponsorManageableDepartments = async (eventId) => {
  try {
    const response = await Api.get(
      `/events/${eventId}/departments/manage-sponsor`
    );
    return response.data;
  } catch (error) {
    console.error(
      `Error fetching sponsor manageable departments for event ID ${eventId}:`,
      error
    );
    throw error;
  }
