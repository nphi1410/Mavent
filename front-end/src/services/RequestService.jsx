...
export const getRequestsByEventIdAndAccountId = async (eventId, accountId) => {
  try {
    const response = await Api.get(
      `/event/${eventId}/request/account/${accountId}`
    );
    return response.data;
  } catch (err) {
...
export const getRequestsByEventIdAndDepartmentId = async (
  eventId,
  departmentId
) => {
  try {
    const response = await Api.get(
      `/event/${eventId}/request/department/${departmentId}`
    );
    return response.data;
  } catch (err) {
...
export const updateRequest = async (updateDTO) => {
  ...
  if (!eventId || !requestId) {
    throw new Error(
      "Event ID and Request ID are required for updating a request."
    );
  }
