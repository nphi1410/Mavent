...
const useMemberData = (eventId = 1, filters = {}, pagination = {}) => {
    ...
    try {
      ...
      if (departmentFilter) {
        ...
        // If it's a numeric string, convert it
          ...
          typeof departmentFilter === "string" &&
          /^\d+$/.test(departmentFilter)
        ) {
    ...
    try {
      ...
      try {
        ...
        if (departmentsData.length > 0) {
          // console.log(
          //   "Successfully received departments data:",
          ...
          console.warn(
            "API returned empty departments array, trying alternative approach..."
          );
          ...
          try {
            ...
            if (altData && Array.isArray(altData)) {
              console.log(
                "Successfully received departments from alternative API"
              );
              ...
              console.log("Alternative API also returned empty data");
              setDepartments([]);
            }
          } catch (altErr) {
  ...
  const findDepartmentByName = useCallback(
    (departmentName) => {
      ...
      let dept = departments.find(
        (d) => d.name.toLowerCase().trim() === deptName
      );
  ...
  const resolveDepartmentFilter = useCallback(
    (filter) => {
      ...
      if (typeof filter === "string" && /^\d+$/.test(filter)) {
        ...
        console.log(
          "Department filter is numeric string, converting to number:",
          numValue
        );
        return numValue;
      }
      ...
      if (dept) {
        ...
        console.log(
          `Could not resolve department name "${filter}", using original value`
        );
