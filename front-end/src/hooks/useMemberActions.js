...
const useMemberActions = (eventId = 1) => {
      ...
      try {
        ...
        // If no departmentId stored, try to match department name
          ...
          const exactMatch = departments.find(
            (dept) => dept.name.toLowerCase().trim() === deptName
          );
  ...
  const unbanMember = useCallback(
    async (member) => {
      try {
        ...
        if (!member || (!member.accountId && !member.id)) {
          console.error(
            "Invalid member object provided to unbanMember:",
            member
          );
          setError("Invalid member data");
          return false;
  ...
  const bulkBanMembers = useCallback(
    async (members) => {
      try {
        setLoading(true);
        setError(null);
        ...
        if (!members || !Array.isArray(members) || members.length === 0) {
          console.error(
            "Invalid members array provided to bulkBanMembers:",
            members
          );
        ...
        return {
          success: true,
          message: "Đã cấm tất cả thành viên thành công",
          results,
        };
      } catch (err) {
