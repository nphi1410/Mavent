...
const DocumentUpload = ({ eventId, departmentId, onUploadComplete }) => {
  ...
  const handleFileSelect = (file) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      setError(
        `File size exceeds 20MB limit. Your file is ${formatFileSize(
          file.size
        )}.`
      );
      showNotification(
        `File size exceeds 20MB limit. Maximum allowed size is 20MB.`,
        "error"
      );
      return;
    }
    ...
    return (
      <div className="border-2 border-dashed rounded-lg p-6 mb-8 text-center border-gray-200 bg-gray-50">
        <p className="text-gray-500">
          Event ID is required to upload documents.
        </p>
      </div>
    );
  }
      ...
      >
        ...
        {!selectedFile ? (
          <>
            ...
            <p className="text-sm text-gray-500 mb-4">or drag file in here</p>
            <button
              type="button"
